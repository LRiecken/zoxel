...
class Vector3:
    ...
    def rotate_around(self, axis, theta):
        ...
        return Vector3((u * dt + x * ct + (-w * y + v * z) * st), (v * dt + y * ct + (w * x - u * z) * st),
...
class Quaternion:
    ...
    def __mul__(self, other):
        if isinstance(other, Quaternion):
            ...
            yz2 = 2 * y * z
            zz = z * z
            return other.__class__(ww * Vx + wy2 * Vz - wz2 * Vy + xx * Vx + xy2 * Vy + xz2 * Vz - zz * Vx - yy * Vx,
    ...
    def new_rotate_matrix(cls, m):
        if m[0 * 4 + 0] + m[1 * 4 + 1] + m[2 * 4 + 2] > 0.00000001:
            ...
            return cls(s * t, (m[1 * 4 + 2] - m[2 * 4 + 1]) * s, (m[2 * 4 + 0] - m[0 * 4 + 2]) * s,
                       (m[0 * 4 + 1] - m[1 * 4 + 0]) * s)
            ...
            return cls((m[1 * 4 + 2] - m[2 * 4 + 1]) * s, s * t, (m[0 * 4 + 1] + m[1 * 4 + 0]) * s,
                       (m[2 * 4 + 0] + m[0 * 4 + 2]) * s)
            ...
            return cls((m[2 * 4 + 0] - m[0 * 4 + 2]) * s, (m[0 * 4 + 1] + m[1 * 4 + 0]) * s, s * t,
                       (m[1 * 4 + 2] + m[2 * 4 + 1]) * s)
            ...
            return cls((m[0 * 4 + 1] - m[1 * 4 + 0]) * s, (m[2 * 4 + 0] + m[0 * 4 + 2]) * s,
                       (m[1 * 4 + 2] + m[2 * 4 + 1]) * s, s * t)
...
def _connect_line3_line3(A, B):
    ...
    if not B._u_in(ub):
        ub = max(min(ub, 1.0), 0.0)
    return LineSegment3(Point3(A.p.x + ua * A.v.x, A.p.y + ua * A.v.y, A.p.z + ua * A.v.z),
...
def _connect_sphere_sphere(A, B):
    ...
    v.normalize()
    return LineSegment3(Point3(A.c.x + s1 * v.x * A.r, A.c.y + s1 * v.y * A.r, A.c.z + s1 * v.z * A.r),
...
def _intersect_line3_sphere(L, S):
    ...
    if not L._u_in(u2):
        u2 = max(min(u2, 1.0), 0.0)
    return LineSegment3(Point3(L.p.x + u1 * L.v.x, L.p.y + u1 * L.v.y, L.p.z + u1 * L.v.z),
